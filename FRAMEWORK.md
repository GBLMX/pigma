...
基准输出现在也随 CI 走：`.github/workflows/ci.yml` 的 `bench` job 在 ubuntu runner 上跑同一条命令，把**原始输出**上传成 artifact（`bench-output`）并写进 job 摘要 —— 共享 runner 的数字只作趋势，本页引用的仍是以作者机器为准的那一档。
