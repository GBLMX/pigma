...
本仓库是 [akirco/pigma](https://github.com/akirco/pigma) 的 fork，**持续维护中**：上游的进展在这里跟进，本仓库自己也带了一批改动 —— 鼠标交互（点击 seek／切区／播放控制／模式／喜欢／静音）、频谱与音高读数、`:` 命令行与补全（含邮箱/短信登录）、听歌打卡、自定义主题（`[themes.<名>]` 继承内置主题，只写要改的颜色）、按终端能力自动适配配色与字形、按终端的图像与输入能力走原生协议(kitty 图形协议封面、同步刷新、kitty 键盘协议与括号粘贴；封面协议可用 `[playerbar] image_protocol` 强制)。二进制与 AUR 包都从**本仓库**发布（[releases](https://github.com/GBLMX/pigma/releases)、`pigma-gblmx-bin`）。
